...
async function ordens(el) {
  ...
  const clienteOptions = clientesForOS.map(c => `<option value="${c.id}" data-rua="${c.endereco||''}" data-numero="${c.numero||''}" data-complemento="${c.complemento||''}" data-cidade="${c.cidade||''}" data-ref="${c.referencia||''}">${c.nome}</option>`).join('');
...
async function enviarNfseWhatsapp(chaveAcesso, telefone, numeroNf, valor, descricao, dataEmissao) {
  if (!chaveAcesso) { toast('Chave de acesso não disponível', 'error'); return; }
  let numeroConfirmado;
  try {
    numeroConfirmado = await modalConfirmarEnvioWA({ telefone, titulo: `Enviar NFS-e Nº ${numeroNf}` });
  } catch (e) {
    console.error('Erro no modal de confirmação:', e);
    toast('Erro ao abrir confirmação', 'error');
    return;
  }
  if (!numeroConfirmado) return;
  try {
    await api('POST', '/whatsapp/enviar-nfse', { telefone: numeroConfirmado, chave_acesso: chaveAcesso, numero_nf: numeroNf, valor, descricao, data_emissao: dataEmissao });
