...
// Executa o fechamento de um mês/ano. Retorna o fechamento criado.
function executarFechamento(mes, ano) {
    const jaExiste = db.prepare('SELECT id FROM fechamentos_comissao WHERE mes = ? AND ano = ?').get(mes, ano);
    ...
    const vendedoresAtivos = db.prepare(`SELECT id, salario_base, meta, bonus_meta FROM vendedores WHERE ativo = 1 AND salario_base > 0`).all();
    ...
    const valesDoMes = db.prepare(`
        SELECT vendedor_id, SUM(valor) as total FROM vales
        WHERE data >= ? AND data <= ? AND fechamento_id IS NULL
    ...
    const insertFech = db.prepare(`
        INSERT INTO fechamentos_comissao (mes, ano, total_geral, total_vales, total_liquido, total_salarios, total_bonus, total_a_pagar) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    const fechId = db.transaction(() => {
        const r = insertFech.run(mes, ano, totalGeral, totalVales, totalLiquido, totalSalarios, totalBonus, totalAPagar);
...
// GET /api/comissoes/parcial?mes=X&ano=Y — prévia sem fechar (padrão: mês atual)
router.get('/parcial', (req, res) => {
    ...
    // Inclui vendedores com salário mesmo sem OS no período
    const todosVendedores = db.prepare(`SELECT id as vendedor_id, nome as vendedor_nome, percentual_comissao, salario_base, meta, bonus_meta FROM vendedores WHERE ativo = 1`).all();

    const jaFechado = !!db.prepare('SELECT id FROM fechamentos_comissao WHERE mes = ? AND ano = ?').get(mes, ano);
    ...
    // Vales por vendedor no período
    const inicioD = inicio.slice(0, 10);
    const fimD    = fim.slice(0, 10);
    const valesResumo = db.prepare(`SELECT vendedor_id, SUM(valor) as total FROM vales WHERE data >= ? AND data <= ? GROUP BY vendedor_id`).all(inicioD, fimD);
    const valesMap = {};
...
// GET /api/comissoes — lista todos os fechamentos
router.get('/', (req, res) => {
    const list = db.prepare(`
        SELECT * FROM fechamentos_comissao ORDER BY ano DESC, mes DESC
    `).all();
    res.json(list);
});

// GET /api/comissoes/:id — detalhe de um fechamento (agrupado por vendedor)
router.get('/:id', (req, res) => {
    const fech = db.prepare('SELECT * FROM fechamentos_comissao WHERE id = ?').get(req.params.id);
...
// DELETE /api/comissoes/:id — remove um fechamento do histórico
router.delete('/:id', (req, res) => {
    const fech = db.prepare('SELECT id FROM fechamentos_comissao WHERE id = ?').get(req.params.id);
    if (!fech) return res.status(404).json({ error: 'Fechamento não encontrado' });
    db.transaction(() => {
        ...
        db.prepare('DELETE FROM fechamentos_comissao WHERE id = ?').run(req.params.id);
    })();
    res.json({ ok: true });
});

// POST /api/comissoes/fechar — fechamento manual (body: { mes, ano })
