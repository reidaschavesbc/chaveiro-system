...
router.get('/', (req, res) => {
    ...
    let sql = `SELECT * FROM lembretes`;
    const params = [];
    if (status) { sql += ` WHERE status = ?`; params.push(status); }
    sql += ` ORDER BY data_envio DESC`;

    const lembretes = db.prepare(sql).all(...params);
    const vendedores = db.prepare(`SELECT id, nome, telefone FROM vendedores WHERE ativo = 1 ORDER BY nome`).all();

    // Enriquece cada lembrete com nomes dos destinatários
...
router.post('/', (req, res) => {
    ...
    const r = db.prepare(
        `INSERT INTO lembretes (mensagem, data_envio, destinatarios) VALUES (?, ?, ?)`
    ).run(mensagem.trim(), data_envio, dest);

    res.json({ id: r.lastInsertRowid, ok: true });
});

// DELETE /api/lembretes/:id — cancela ou remove
router.delete('/:id', (req, res) => {
    const l = db.prepare('SELECT * FROM lembretes WHERE id = ?').get(req.params.id);
